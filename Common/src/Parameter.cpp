...
void Parameter::createParameter(DB_mem* p, const std::string& name,
	...
	if (p->nrParameters == maxNrParameters)
		throw std::runtime_error(
				"Exceeding memory space while creating Parameter: " + name);
	...
	if (idx < 0) {
		...
		if (name.length() > MaxNameLength - 1) {
			std::clog << "Warning: only using " << MaxNameLength - 1
					<< " characters from: " << name << std::endl;
		}
...
Parameter& Parameter::operator=(const double other) {
	set(other);
	return *this;
}
